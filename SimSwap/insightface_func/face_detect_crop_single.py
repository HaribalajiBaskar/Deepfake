...
class Face_detect_crop:
    def __init__(self, name, root='~/.insightface_func/models'):
        self.models = {}
        root = os.path.expanduser('/content/SimSwap/insightface_func/models')
        print(os.listdir('/content/SimSwap/insightface_func/models'))
        print(name)
        onnx_files = glob.glob(os.path.join(root, name, '*.onnx'))
        print(onnx_files)
